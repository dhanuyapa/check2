...
function DeleteAccount() {
  ...
  const handleRemoveAccount = () => {
    ...
    if (confirmDelete) {
      ...
      axios
        .delete(`http://localhost:8070/customers/deleteCus/${loggedInUserNIC}`)
        .then((response) => {
          // Handle success (account deleted)
          alert('Account removed successfully');
          // Navigate to the registration page
          navigate('/add'); // Use navigate to change the route
        })
        .catch((error) => {
          // Handle error
  ...
  return (
    ...
    <div className="delete-account-container"> {/* Add a class for styling */}
